...
async function handleAppUrlOpen(cb) {
  if (!isNative) return;
  App.addListener('appUrlOpen', (data) => {
    if (data && data.url) cb(data.url);
  });
  // عند فتح التطبيق من الصفر عبر رابط (كان مغلقاً تماماً)، حدث appUrlOpen قد
  // يكون فات قبل تسجيل المستمع أعلاه — getLaunchUrl تُرجع نفس الرابط في هذه الحالة
  try {
    const launch = await App.getLaunchUrl();
    if (launch && launch.url) cb(launch.url);
  } catch (e) {}
}
